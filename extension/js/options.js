(function($) {
  ...
  function updatePasswordPreview() {
    ...
    if ($("#type").val() == "SGPP") pass = "oJ}HInHYCxAAshKSVShgv379";

    $(".password-preview .generatedPass").text(
      pass.substr(0, $("#length").val() || 4)
    );
  }
  ...
  function addPassword(pass) {
    ...
    $("#saved_passwords tbody").append(
      "<tr>" +
        ...
        "</td>" +
        "<td>" +
        boolIcons[Boolean(pass.hash)] +
        "</td>" +
        "<td>" +
        boolIcons[Boolean(pass.stealth)] +
        "</td>" +
        "<td>" +
        pass.length +
        "</td>" +
        "<td>" +
        (pass.always ? "Always" : pass.session ? "Session" : "No") +
        "</td>" +
        '<td><button class="remove" data-id="' +
        pass.id +
        '">Remove</button></td>' +
        "</tr>"
    );
  ...
  $(function() {
    ...
    $("body")
      .on("mouseenter", ".fld input", function() {
        $(this)
          .parents(".fld")
          .addClass("hover");
      })
    ...
    $("input[type=range]")
      .on("change", function() {
        ...
        rangeValue.css({
          left: (this.clientWidth / this.max) * this.value + "px"
        });
      })
      .trigger("change");
